...
PopupMenuLinks.prototype.rebind = function () {
  var childElement, menuElement, menuItem, textContent, numItems, label;
  this.menuitems = [];
  // Traverse the element children of domNode: configure each with
  // menuitem role behavior and store reference in menuitems array.
...
PopupMenuLinks.prototype.setFocusToPreviousItem = function (currentItem) {
  ...
  if (currentItem === this.firstItem) {
    this.lastItem.domNode.focus();
       ...
       index = this.menuitems.indexOf(currentItem);
	   if(this.menuitems[index - 1].domNode === $(".menuitem")[$(".menuitem").length-2]){
		     isJump = false;
		}
	}
};
...
PopupMenuLinks.prototype.open = function () {
  ...
  $(".progress, .pageaction, #header-btn-container, main, footer").attr("aria-hidden", true);
};
...
PopupMenuLinks.prototype.close = function (force) {
  ...
  if (force || (!this.hasFocus && !this.hasHover && !this.controller.hasHover)) {
    ...
    this.controller.domNode.children[0].src = 'scripts/external/menu/ico_menu-v1.png';

    $(".progress, .pageaction, #header-btn-container, main, footer").removeAttr("aria-hidden");
  }
};
